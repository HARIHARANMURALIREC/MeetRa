...
-- Project: https://supabase.com/dashboard/project/nzrxeogmkxdhmyoxylnk/sql/new
-- Safe to re-run.
...
create index if not exists idx_participants_room_user on public.participants(room_id, user_id);
create index if not exists idx_chat_messages_room_created on public.chat_messages(room_id, created_at);

-- Helper functions live in an unexposed schema so they are not callable over the Data API,
-- while still being usable from RLS policies.
...
-- v1.1–v1.3 features: also run supabase/migrations/20260806140000_v11_v13_features.sql
